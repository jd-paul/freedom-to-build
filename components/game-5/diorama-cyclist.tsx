...
interface DioramaCyclistProps {
  helmetColor: string
  getTerrainHeight: (x: number, z: number) => number
}

export function DioramaCyclist({ helmetColor, getTerrainHeight }: DioramaCyclistProps) {
  ...
  useFrame((_, rawDelta) => {
    ...
    g.position.y = getTerrainHeight(g.position.x, g.position.z) + 0.1
    ...
    _cameraPos.set(
      g.position.x + Math.sin(g.rotation.y + Math.PI) * 9,
      g.position.y + 5,
      g.position.z + Math.cos(g.rotation.y + Math.PI) * 9,
    )
    camera.position.lerp(_cameraPos, 1 - Math.exp(-delta * 3.2))
    _cameraTarget.set(
